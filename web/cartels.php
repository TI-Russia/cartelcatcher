...
// $r="<table border=1 cellspacing=0 cellpadding=0><tr>";
 $r="";
 $sql="select count(*) as cnt,reg from contractslt where oid=".$id." group by reg order by reg";
...
// $r="<table border=1 cellspacing=0 cellpadding=0><tr>";
 $r="";
// $sql="select count(*) as cnt,reg from contractslt where oid in (select oid from  orgs where gid=".$id.") group by reg order by reg";
 $sql="select cnt,reg from cartelregs where cid=".$id." order by reg";
 $stmt = sqlsrv_query ($db, $sql);
  if( $stmt === false ) {
    echo ($sql.'<br>');
