...
if (($oid=="")&&($phone=="")&&($email=="")&&($price=="")&&($cinn=""))
   {    
    ...
    {
     if ($oid>0) {$sql="select * from contractsLT where oid=".$oid;
//echo $sql."<br>";
...
else if ($phone!='') { $sql="select [id],[contractsLT].[oid],[contractnumber],[contactphone],[contactemail],[purchasenumber],[name],[inn],[price],[date],[ver],[coid] from contractsLT join orgs on orgs.oid=contractsLT.oid where contactphone='".$phone."'";}
else if ($email!='') { $sql="select [id],[contractsLT].[oid],[contractnumber],[contactphone],[contactemail],[purchasenumber],[name],[inn],[price],[date],[ver],[coid] from contractsLT join orgs on orgs.oid=contractsLT.oid where contactemail='".$email."'";}
else if ($coid!='')  { $sql="select [id],[contractsLT].[oid],[contractnumber],[contactphone],[contactemail],[purchasenumber],[name],[inn],[price],[date],[ver],[coid] from contractsLT join orgs on orgs.oid=contractsLT.oid where coid='".$coid."'";
	echo '<table border=2><tr><td><center><table border=1><tr><td>'.$coidname.'</td></tr></table></td></tr><tr><td>';}
else if ($price!='') { $sql="select [id],[contractsLT].[oid],[contractnumber],[contactphone],[contactemail],[purchasenumber],[name],[inn],[price],[date],[ver],[coid] from contractsLT join orgs on orgs.oid=contractsLT.oid where price='".$price."'";} 
else if ($id!='')    { $sql="select [id],[contractsLT].[oid],[contractnumber],[contactphone],[contactemail],[purchasenumber],[name],[inn],[price],[date],[ver],[coid] from contractsLT join orgs on orgs.oid=contractsLT.oid where contractnumber='".$id."'";};
if (($oid!='')&&($coid!=''))
